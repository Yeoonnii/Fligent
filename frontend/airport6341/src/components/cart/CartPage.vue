<template>
    <div class="container">
        ...
        <div class="butt">
            ...
            <el-button link @click="handleDeletebatch()" type="danger" style="font-weight: 600; font-size: 18px; float: right; margin-right: 20px;">체크 삭제</el-button>
...
export default {
    setup () {
        ...
        const handleData = async() => {
            const url = `/ROOT/api/cart/selectcartlist.json`;
            const headers = {
        ...
        const handleDelete = async(cno) => {
            if(confirm('삭제할까요?')){
                const url = `/ROOT/api/cart/deletecart.json?cno=${cno}`;
                const headers = {
                    "Content-Type":"application/json", 
                    "token" : state.token
                };
        ...
        const handleDeletebatch = async() => {
            if(confirm('선택한 상품을 삭제할까요?')){
                const url = `/ROOT/api/cart/deletebatchcart.json`;
                // console.log(url)
                const headers = {
