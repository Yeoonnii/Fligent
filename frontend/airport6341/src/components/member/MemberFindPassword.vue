<template>
    <div class="container">
        ...
        <br />
        <hr />
        <div v-if="state.sendresult == 1" style="width: 80%; margin: auto; padding: 20px;">
        <el-form-item label="이메일로 발송된 인증코드를 입력하세요">
            <el-input style="width:200px" v-model="state.userinputcode" :ref = "el => {form[5] = el}"/>
            <el-button round @click="handleCodeCheck()" style="margin-left: 20px; font-size: 15px; font-weight: 600;" >인증코드 확인</el-button>
        </el-form-item>
    </div>
    <div v-if="state.codeckeckret == 1">
        <el-form :model="form" label-width="120px">
            ...
            <el-form-item label="변경할 암호 확인">
                <el-input type="password" style="width:200px" v-model="state.newpw1" :ref = "el => {form[7] = el}"/><br />
            </el-form-item>
            
            <el-button color="#F76868" link @click="handleSetPw()" style="font-size: 15px; font-weight: 600;" >암호 변경하기</el-button>
        </el-form>
    </div>    
    </div>
</template>
...
export default {
    setup () {
        ...
        const state = reactive({
            ...
            codeckeck : ' 인증코드입력 ', // 아이디 중복확인처럼 변경하고싶어요ㅜ
        ...
        const handleSendMail = async() => {
            ...
            const url = `/ROOT/api/member/sendmail.json`;
        ...
        const handleSetPw = async() => {
            ...
            const url = `/ROOT/api/member/setnewpw.json`;
            const headers = {"Content-Type":"application/json"};
            const body = {
                userid  : state.userid,
                userpw  : state.newpw
