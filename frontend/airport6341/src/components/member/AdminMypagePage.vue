        ...
        <el-tabs tab-position="left" class="demo-tabs">
            ...
            <h3>회원목록</h3>
                <el-table :data="state.list" style="width: 90%; text-align: center;">
                    <el-table-column fixed prop="userid" label="아이디" width="120" />
                    <el-table-column prop="nickname" label="닉네임" width="120" />
                    <el-table-column prop="block" label="차단유무" width="120" />
                    ...
                    <el-table-column fixed="right" label="회원관리" width="120">
                        <template #default="scope">
                            ...
                            <el-button link type="primary" v-if="state.list[scope.$index].block === 0" 
                                @click="handleBlock(scope.row.userid)" style="font-weight: 600; color: red;">차단</el-button>
                            <el-button link type="primary" v-else-if="state.list[scope.$index].block === 2"
            ...
            <el-tab-pane label="회원 게시물관리" style="margin-left: 50px;">
            <h3>회원 게시물목록</h3>
                <el-table :data="state.list1" style="width: 94%; text-align: center;">
                    <el-table-column fixed prop="memberUserid" label="회원아이디" width="150" />
                    <el-table-column prop="memberNickname" label="닉네임" width="150" />
                    <el-table-column prop="bno" label="번호" width="120" />
                    <el-table-column prop="title" label="제목" width="150" />
                    <el-table-column prop="content" label="내용" width="150" />
                    <el-table-column prop="hit" label="조회수" width="120" />
                    <el-table-column fixed="right" label="게시글관리" width="120">
                        <template #default="scope">
            ...
            <el-tab-pane label="상품목록" style="margin-left: 70px;">
                <h3>상품목록</h3>
                <div style="width: 80%;">
                ...
                <el-table :data="state.list2" style="width: 90%">
                    <el-table-column prop="ino" label="상품번호" width="130" />
                    <el-table-column prop="name" label="상품명" width="150" />
                    <el-table-column prop="price" label="가격" width="130" />
                    <el-table-column prop="quantity" label="재고수량" width="120" />
                    <el-table-column prop="regdate" label="등록일" width="120" />
                    <el-table-column fixed="right" label="상품관리" width="120">
                        <template #default="scope">
                            <el-button text class="button" type="danger" style="font-weight: 600; margin-left: 10px;" @click="handleDeleteitem(scope.row.ino)" >삭제하기</el-button>
                            <el-button text class="button" type="primary" style="font-weight: 600;" @click="handleUpdate(scope.row.ino)" >수정하기</el-button>
                        </template>
                    </el-table-column>
...
export default {
    setup () {
        ...
        const handleData = async() => {
            const url = `/ROOT/api/admin/mypage/selectmember.json?page=${state.page}`;
        ...
        const handleData1 = async() => {
            const url = `/ROOT/api/admin/mypage/selectboard.json?page=${state.page1}`;
            const headers = {"Content-Type":"application/json", "TOKEN" : state.token};
            const { data } = await axios.get(url, {headers});
            console.log(state.token)
            if(data.status === 200) {
        ...
        const handleData2 = async() => {
            const url = `/ROOT/api/admin/item/selectitem.json?page=${state.page2}&icateno=${state.icateno}`;
            const headers = {"Content-Type":"application/json", "TOKEN" : state.token};
            const { data } = await axios.get(url, {headers});
            if(data.status === 200) {
        ...
        const handleBlock = async(userid) => {
            const url = `/ROOT/api/admin/mypage/blockmember.json?userid=${userid}`;
            const headers = {"Content-Type":"application/json", "TOKEN" : state.token};
            const body = {
        ...
        const handleDeleteboard = async(bno) => {
            if(confirm('삭제할까요?')){
                const url = `/ROOT/api/admin/mypage/deleteboard.json?bno=${bno}`;
                const headers = {"Content-Type":"application/json", "TOKEN" : state.token};
                const body = {
        ...
        const handleDeleteitem = async(ino) => {
            if(confirm('삭제할까요?')){
                const url = `/ROOT/api/admin/item/delete.json?itemno=${ino}`;
                const headers = {"Content-Type":"application/json", "TOKEN" : state.token};
                const body = {
