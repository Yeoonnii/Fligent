            ...
            <el-tab-pane label="회원정보 수정" name="userInfoChange">
                ...
                <div class="change">
                    <el-form class="info" label-width="150px">
                        ...
                        <br />
                        <el-form-item>
                            <el-button type="primary" round @click="handleUpdate" style="margin-left: 200px; font-size: 20px;">수정하기</el-button>
                        </el-form-item>
                    </el-form>
                </div>
            </el-tab-pane>
            ...
            <el-tab-pane label="나의 활동" style="margin-left: 50px;">
                ...
                <el-tabs tab-position="top" type="border-card" class="demo-tabs" style="border: 1px solid #ffffff">
                    ...
                    <el-tab-pane label="좋아요한 글">
                        <el-table :data="state.llist" style="width: 80%; margin-left: 100px;">
                            <el-table-column prop="title" label="제목" />
                            <el-table-column prop="content" label="내용" />
                            <el-table-column prop="hit" label="조회수" />
                            <el-table-column prop="regdate" label="작성일" />
...
export default {
    setup () {
        ...
        const handleIMGUpdate = async() => {
            const url = `/ROOT/api/customer/selectmemberimgupdate.json`;
            const headers ={
        ...
        const handleList = async() => {
            const url =`/ROOT/api/order/selectorderlist.json?start=${state.page}`;
            const headers = {
                "Content-Type":"application/json",
                "TOKEN" : state.token
            };
        ...
        const handleCancle = async(ono) => {
            if(confirm('주문 취소할까요?')){
                const url= `/ROOT/api/order/deleteorder.json?ono=${ono}`;
                const headers = {
                    "Content-Type":"application/json",
                    "token" : state.token
                };
        ...
        const handleBoard = async() => {
            const url =`/ROOT/api/customer/mypage/boardlist.json?start=${state.page1}`;
            const headers = {
                "Content-Type":"application/json",
                "TOKEN" : state.token
            };
        ...
        const handleDeleteboard = async(bno) => {
            if(confirm('삭제할까요?')){
                const url = `/ROOT/api/board/deleteoneboard.json?bno=${bno}`;
                const headers = {
                    "Content-Type":"application/json", 
                    "TOKEN" : state.token
                };
        ...
        const handleReply = async() => {
            const url =`/ROOT/api/customer/mypage/boardreplylist.json?start=${state.page2}`;
            const headers = {
                "Content-Type":"application/json",
                "TOKEN" : state.token
            };
        ...
        const handleLike = async() => {
            const url =`/ROOT/api/customer/mypage/boardlikelist.json?start=${state.page3}`;
            const headers = {
                "Content-Type":"application/json",
                "TOKEN" : state.token
            };
        ...
        const handleUnlike = async(bno) => {
            if(confirm('해제할까요?')){
                const url = `/ROOT/api/customer/mypage/deletelike.json?bno=${bno}`;
                const headers = {
                    "Content-Type":"application/json", 
                    "TOKEN" : state.token
                };
        ...
        const handleUpdatePw = async() => {
            ...
            const url = `/ROOT/api/customer/mypage/mypageupdatepassword.json`;
            const headers = {
                "Content-Type":"application/json",
                "token" : state.token
            };
        ...
        const handleDelete = async() => {
            ...
            if(confirm('탈퇴할까요?')){
                const url= `/ROOT/api/customer/mypage/delete.json`;
                const headers = {
                    "Content-Type":"application/json",
                    "token" : state.token
                };
