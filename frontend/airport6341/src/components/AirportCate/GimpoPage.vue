    ...
    <div class="traffic">
      <div class="row">
        ...
        <div class="col-6">
          ...
          <el-descriptions direction="vertical" v-for="obj of state.parkinginfodata" :key="obj" border="1" style="width: 90%; margin: auto; height: 85px; margin-right: 100px;" >
...
export default {
    setup () {
      ...
      const handleData1 = async() => {
          const url = `/ROOT/api/airportstore/selecttypeofbussiness.json?airportCode=GMP&typeOfBusiness=${state.typeofbusiness}&start=${state.page}`;
          const headers = {"Content-Type":"application/json"};
          const {data} = await axios.get(url, {headers});
          // console.log(data);
          if(data.status === 200){
