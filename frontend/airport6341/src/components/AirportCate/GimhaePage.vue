        ...
        <div class="row">
          ...
          <div class="col-6">
            <h2 style="margin-bottom: 50px;">공항 주차장 혼잡도</h2>
              <el-descriptions direction="vertical" v-for="obj of state.parkinginfodata" :key="obj" border="1" style="width: 90%; margin-top: 30px; height: 85px; margin-right: 100px;">
...
export default {
    setup () {
      ...
      const handleData1 = async() => {
          const url = `/ROOT/api/airportstore/selecttypeofbussiness.json?airportCode=PUS&typeOfBusiness=${state.typeofbusiness}&start=${state.page}`;
          const headers = {"Content-Type":"application/json"};
          const {data} = await axios.get(url, {headers});
          // console.log(data);
          if(data.status === 200){
