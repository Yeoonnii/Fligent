<template>
  <div>
    ...
    <!-- <a href="https://www.flaticon.com/kr/free-animated-icons/" title="화살 애니메이션 아이콘">화살 애니메이션 아이콘 제작자: Freepik - Flaticon</a> -->
    <div class="container">
      <div class="header">
        ...
        <div class="text" style="font-family: 'MapoFlowerIsland';">
          <p class="animate__animated animate__fadeInDown">Fligent</p>
        </div>

        <div class="textt" style="font-family: 'MapoFlowerIsland';">
          <p class="animate__animated animate__fadeInUp">당신을 위한 맞춤 공항정보 |플라이전트|</p>
        </div>
      </div>
      <div class="boarding">
        <div class="row">
          <div class="col-8" style="height: 30vh; text-align: center;">
            <select class="form-select" v-model="state.cate" 
                style="width: 180px; margin-left: 20px; font-size: 20px; font-weight: 600;" 
            ...
            <h2 style="margin-top: 100px; margin-left: 200px;">탑승수속 예상 소요시간 <i class="bi bi-hourglass-split"></i></h2>
            <h6 style="margin-left: 200px; color: #cccccc;">*인천공항과 김해공항에서는 제공되지 않는 정보입니다.</h6>
          </div>

          <div class="col-4" style="margin: auto;">
            <div class="card" style="width: 70%; height: 50%; border: 1px solid #ffffff; background-color: #F2E9E9;">
              <div class="card-body">
                <h5 class="card-title" style="font-size: 27px;">오늘의 날씨<i class="bi bi-cloud-sun"></i></h5>
                ...
                <div class="card-text">
                  <table class="table table-borderless">
                    ...
                    <tbody class="table-group-divider" style="text-align: center; font-size: 16px;">
      ...
      <div class="info">
        <div class="row" style="margin-top: 50px;">
          ...
          <div class="col-6" style="width: 45%;">
            <h2>현재 공항 운항 일정</h2>
            <h6 style="margin-left: 200px; color: #cccccc;">*인천공항에서는 제공되지 않는 정보입니다.</h6>
            ...
            <select class="form-select"
                  style="float: left; width: 200px; margin-top: 10px; margin-right: 10px; margin-bottom: 10px;" 
            ...
            <table style="width:100%; height: 540px; text-align: center; font-size: 15px; margin-bottom: 5px;">
              ...
              <tbody v-if="state.flightstatuslistdatadata != null">
      ...
      <div class="hotboard">
        <div>
          <el-button color="#0339A6" round style="margin-left: 45px; height: 40px; margin-bottom: 10px; font-size: 20px; font-weight:600;">#조회수 많은</el-button><r />
          <el-button link class="button1" @click="handleBoard()" style="float: right; font-size: 18px; margin-top: 8px; margin-right: 50px; font-weight: 600;"> <i class="bi bi-blockquote-left fs-4"></i>더 많은 글 ></el-button>
        </div>
        ...
        <el-row>
          <div class="card" v-for="obj of state.hit" :key="obj">
            ...
            <div class="card-body">
              <h5 class="card-title">{{obj.title}}</h5><hr />
              <p class="card-text" style="font-size: 18px;">{{obj.content}}</p>
              <p class="card-text" style="font-size: 12px;">{{obj.regdate}}</p>
              <p class="card-text"><span style="color:#cccccc;">by. </span><span style="font-weight: 600;">{{obj.nickname}}</span>
                <img src="../assets/free-animated-icon-view-8121254.gif" style="width: 30px; margin-left: 75px;" />{{obj.hit}}
        ...
        <div style="margin-top: 10px">
          <el-button color="#F2D1C9" round style="margin-left: 45px; height: 40px; margin-bottom: 10px; font-size: 20px; font-weight:600;">#인기 많은</el-button><r />
        </div>
        ...
        <el-row>
          <div class="card" v-for="obj of state.like" :key="obj">
            <img style="height: 170px;" :src="obj.imgurl" class="card-img-top"/>
            <div class="card-body">
              <p class="card-title" style="font-size: 20px;">{{obj.title}}</p><hr />
              <p class="card-text" style="font-size: 18px;">{{obj.content}}</p>
              <p class="card-text" style="font-size: 12px;">{{obj.regdate}}</p>
              <p class="card-text"><span style="color:#cccccc;">by. </span><span style="font-weight: 600;">{{obj.nickname}}</span>
                <img src="../assets/free-animated-icon-heart-8121302.gif" style="width: 30px; margin-left: 75px;" />{{obj.lcnt}}
                <i class="bi bi-chat-left-dots fs-6" style="margin-left: 10px;"></i> {{obj.recnt}}
              </p>
              <div class="bottom">
    ...
    var hours = now.getHours();	// 시간
      ...
      const handleUser = async() => {
        if(state.token == null){return;}
          const url =`/ROOT/api/customer/mypage/update.json`;
          const headers = {
      ...
      const handleData = async() => {
        // 한국공항공사_항공기 운항정보
        const url = `/ROOT/api/flightstatuslist.json?pageNo=${state.flightstatuslistpageNo}&schAirCode=${state.cate}&type=${state.type}&schStTime=${state.schStTime}&schEdTime=${state.schEdTime}`;
        ...
        // 한국공항공사_공항 소요시간 정보
        const url1 = `/ROOT/api/getwaittime.json?cate=${state.cate}`;
      ...
      const youtubeUrlDATA = async() => {
        const url = `/ROOT/api/airportyoutube/selectyoutubeurl.json?airportCode=${state.cate}`;
        const headers = {"Content-Type" : "application/json"};
        const {data} = await axios.get(url,{headers});
      ...
      const url = `/ROOT/api/weatherinfo.json?schAirCode=${state.cate}`;
      const headers = {"Content-Type" : "application/json"};
      const data = await axios.get(url, {headers});
        if(data.data !=null){
      ...
      const handleBoardHit = async() => {
        const url = `/ROOT/api/board/boardHitselect.json`;
        const headers = {"Content-Type" : "application/json"};
        const { data } = await axios.get(url, {headers});
      ...
      const handleBoardLike = async() => {
        const url = `/ROOT/api/board/boardLikeselect.json`;
        const headers = {"Content-Type" : "application/json"};
        const { data } = await axios.get(url, {headers});
          console.log(data.result)
          if(data.status === 200){
